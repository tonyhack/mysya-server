...
int main(int argc, char *argv[]) {
  ...
  if (::tutorial::orcas::gateway::server::Config::GetInstance()->Load(
        option.conf_file_) == false) {
      MYSYA_ERROR("Config::Load(%s) failed.", option.conf_file_.data());
      return -1;
  }

  ::mysya::ioevent::SocketAddress listen_addr(
      ::tutorial::orcas::gateway::server::Config::GetInstance()->listen_host_,
      ::tutorial::orcas::gateway::server::Config::GetInstance()->listen_port_);

  ::mysya::ioevent::EventLoop event_loop;
  std::unique_ptr< ::tutorial::orcas::gateway::server::AppServer> app_server(
      new (std::nothrow) ::tutorial::orcas::gateway::server::AppServer(&event_loop, 128));
  ...
  if (app_server->Listen(listen_addr) == false) {
    MYSYA_ERROR("AppServer::Listen(%s:%d) failed.",
        ::tutorial::orcas::gateway::server::Config::GetInstance()->listen_host_.data(),
        ::tutorial::orcas::gateway::server::Config::GetInstance()->listen_port_);
    return -1;
  }
